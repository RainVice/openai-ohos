...
`openai_ohos` 的业务调用方式以官方 OpenAI SDK 为准。平台适配仅在内部初始化，不在根入口导出额外的 fetch、初始化函数或取消控制器工厂。普通请求使用全局 `new AbortController()` 与请求选项 `signal`；流式辅助方法使用官方 `stream.abort()`。缺失的标准能力与类型声明由库提供，不能要求应用改用平台专用 API。验证必须包含独立 HAR 消费工程，确保类型声明与运行时一起交付。尚未实现的标准能力应记录为兼容性缺口，不能通过新增自定义公开接口宣称兼容。
...
如需明确重新提交同版本的审核材料，可在重新打包后使用 `npm run publish:ohpm -- --resubmit`。它仅跳过本地版本查重，仍校验产物完整性与元数据；是否允许同版本重提由 OHPM 服务端决定。自动工作流不传此参数，继续保留版本查重。
